...
interface DimensionPromptEditorProps {
  projectName: string
}
...
const PROMPT_TEMPLATES = [
  {
    name: 'query_generation.txt',
    displayName: 'Query Generation',
    description: 'Template for generating queries from dimension tuples'
  },
  {
    name: 'tuple_generation.txt',
    displayName: 'Tuple Generation',
    description: 'Template for generating tuples from dimension combinations'
  }
]

export function DimensionPromptEditor({ projectName }: DimensionPromptEditorProps) {
  ...
  const loadPrompts = async () => {
    ...
    try {
      ...
      for (const template of PROMPT_TEMPLATES) {
        try {
          const response = await fetch(`/api/projects/${projectName}/prompts/${template.name}`)
          if (response.ok) {
  ...
  const saveTemplate = async (templateName: string) => {
    ...
    try {
      ...
      const response = await fetch(`/api/projects/${projectName}/prompts/${templateName}`, {
        method: 'PUT',
        headers: {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Customize the AI prompts used for tuple and query generation
          </p>
        </div>
      </div>
